...
public class ChocolateDistribution {
	...
	static int minimum(int ar[],int m)
	{
		Arrays.sort(ar);
		int min_dif = ar[ar.length-1];
		for(int i=0; i+m-1<ar.length; i++ )
		{
			if(ar[i+m-1] - ar[i] < min_dif)
			{
				min_dif = ar[i+m-1] - ar[i];
			}
		}
		return min_dif;
	}
	public static void main(String[] args) {
		int ar[] = {12, 4, 7, 9, 2, 23, 25, 41,
				30, 40, 28, 42, 30, 44, 48,
				43, 50};
		System.out.print(minimum(ar,7));
	}
